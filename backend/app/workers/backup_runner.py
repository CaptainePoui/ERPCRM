"""Runner du backup cloud infra ERPCRM (TASK-035). Un seul dump physique par
...
- run_scheduled() : appele par cron a intervalle court, evalue quels
...
# Capacite de reconstruction complete du serveur (TASK-035, demande de
# Philippe 2026-08-15) : serveur neuf + restauration de cette archive +
# ajustement des IPv4 doit suffire a remonter ERPCRM. Contient donc, en plus
# de la DB et des uploads, tout ce qui n'est PAS deja dans git : secrets
# (.env), certificats TLS s2s ERPCRM<->SIPV, unites systemd, config Nginx.
# Pas de chiffrement separe de l'archive (decision explicite de Philippe,
# 2026-08-15) -- le serveur lui-meme est deja le vecteur d'attaque principal
# et expose ces memes secrets en clair.
CONFIG_PATHS = [
    (BACKEND_DIR / ".env", "config/backend.env"),
    (BACKEND_DIR / "certs", "config/certs"),
    (Path("/etc/systemd/system/erpcrm-backend.service"), "config/systemd/erpcrm-backend.service"),
    (Path("/etc/systemd/system/erpcrm-backend-tls.service"), "config/systemd/erpcrm-backend-tls.service"),
    (Path.home() / ".config/systemd/user/erpcrm-frontend.service", "config/systemd/erpcrm-frontend.service"),
    (Path("/etc/nginx/sites-available/portail.simpleip.tel"), "config/nginx/portail.simpleip.tel"),
]


def _build_dump() -> Path:
    """pg_dump + uploads/ + config serveur (voir CONFIG_PATHS) dans une seule
    archive tar.gz temporaire."""
    parsed = urlparse(settings.DATABASE_URL_SYNC)
    tmp_dir = Path(tempfile.mkdtemp(prefix="erpcrm_backup_"))
    sql_path = tmp_dir / "erpcrm_db.sql"
    ...
    archive_path = tmp_dir / "erpcrm_backup.tar.gz"
    with tarfile.open(archive_path, "w:gz") as tar:
        tar.add(sql_path, arcname="erpcrm_db.sql")
...
def _remote_filename(frequency_type: str, when: date) -> str:
    return f"erpcrm_{frequency_type}_{when.isoformat()}.tar.gz"
...
async def _rotate_and_upload(db, connection: CloudBackupConnection, cycle: BackupCycle, archive_path: Path, manual: bool):
    ...
    prefix = f"erpcrm_{cycle.frequency_type}_"
    ...
    async with AsyncSessionLocal() as db:
        ...
        try:
            for connection in connections:
                ...
                # connexion. Reessayer indefiniment chaque minute sur une
                # erreur permanente (ex. scope OAuth manquant) inonderait le
                # journal et l'API du fournisseur pour rien (bug trouve le
                # 2026-08-14 : 26+ tentatives identiques en boucle avant ce fix).
                ...
                if already_attempted.scalars().first():
                    continue  # deja tente aujourd'hui pour cette connexion (succes ou echec)
