...
# et la decision documentee dans le commit "Graphiti 100% local"), et fixe le
# contexte Ollama a 32768 tokens (sinon 4096 par defaut sans VRAM, insuffisant
# pour le pipeline Graphiti -- voir ollama-context-length.conf).
#
...
sudo install -m 644 "$SCRIPT_DIR/ollama-context-length.conf" /etc/systemd/system/ollama.service.d/10-context-length.conf
